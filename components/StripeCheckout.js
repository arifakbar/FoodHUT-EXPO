...
import { CardForm, useConfirmPayment, useStripe } from '@stripe/stripe-react-native';
    ...
    return (
        <View style={styles.container}>
            {loading ?
                <Loading /> :
                <>
                    <Text style={{ ...styles.btn, backgroundColor: "#DA9816" }} >Total: Rs. {cartTotal}</Text>
                    <Text style={{ ...styles.btn, backgroundColor: "lightgreen" }} >Payable: Rs. {payable / 100}</Text>
                    <CardForm
                    ...
                    />
                    <TouchableOpacity style={{ ...styles.btn, backgroundColor: "blue" }} onPress={handlePress}>
                        <Text style={{ textAlign: "center", color: "white", fontSize: 15 }}>Pay</Text>
                    </TouchableOpacity></>}
        </View>
    )
}
...
export default connect(mapStateToProps, { couponAppliedAction, addToCartAction })(StripeCheckout);
