            ...
            <>
                ...
                <View style={styles.content}>
                    ...
                    <View style={styles.one}>
                        ...
                        {product.veg ? <TouchableOpacity style={styles.veg}><Text style={{ color: "white" }} >VEG</Text></TouchableOpacity> : <TouchableOpacity style={styles.veg}><Text style={{ color: "white" }} >NON-VEG</Text></TouchableOpacity>}
                    </View>
                    <View style={styles.cat}>
                        <Text style={styles.title}>Category: </Text>
                        {product.category && <Text style={styles.catText}>{product.category.name}</Text>}
                    </View>
                    <View style={styles.subcat}>
                        <Text style={styles.title2}>Sub Category: </Text>
                        {product.subCategory && product.subCategory.map(s => {
                            return <Text key={s._id} style={styles.subCat}>{s.name}</Text>
                        })}
                    </View>
                    <TouchableOpacity style={styles.btn} onPress={() => addToCart(product)}>
                        <Text style={styles.btnText} >Add To Cart</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.btn2} onPress={() => addToWishlist(product._id)}>
...
export default connect(mapStateToProps, { addToCartAction })(SingleProductScreen);