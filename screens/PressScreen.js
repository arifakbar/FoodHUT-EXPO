...
function PressScreen(props) {
    ...
    return (
        <ImageBackground source={require("../assets/images/news.jpg")} style={styles.conatiner} imageStyle={{ opacity: 0.5 }}>
            {loading ? <Loading /> :
                <FlatList style={{ width: "85%" }} keyExtractor={i => i._id} data={news} renderItem={i => {
                    return <View style={styles.main}>
                        <Text style={styles.date}>Posted on {new Date(i.item.createdAt).toDateString()}</Text>
